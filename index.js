...
btmenu1.addEventListener("click", function () {
    ...
    const miPromesa = () => {
        return new Promise((resolve, reject) => {
            resolve(JSON.parse(localStorage.getItem("menuSemanal")));
        })
    }
    miPromesa().then((rest) => {
        Swal.fire({
            ...
            color: `#ff914d`,
            html:
                `El menú del día Lunes es :<br>` + rest[0].ingrediente1 + `<br>` + rest[0].ingrediente2 + `<br>` + rest[0].ingrediente3 + `<br>$` + rest[0].precio,
